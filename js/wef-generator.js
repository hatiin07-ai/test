...
const WG_BOOKMARKLET = "javascript:(async()=>{const e=\"https://uootyovhokziobarpeed.supabase.co/functions/v1/wef-import\",t=\"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVvb3R5b3Zob2t6aW9iYXJwZWVkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI4NTU2MDIsImV4cCI6MjA5ODQzMTYwMn0.lQm3PLyoj5ELktvcuUuVbidh4xmv8W2Oo4uUddQrJk4\",o={\"Content-Type\":\"application/json\",\"X-Import-Token\":\"j7W7edUfwOCoKa08DyyiXjtNVLtUG-HpdTTlDzRmzQg\",apikey:t,Authorization:\"Bearer \"+t};if(\"weflab.com\"!==location.hostname)return void alert(\"⚠️ weflab.com 후원관리 페이지에서 실행하세요.\");const n=(document.cookie.match(/login_idx=([^;]+)/)||[])[1];if(!n)return void alert(\"⚠️ login_idx 없음. weflab 로그인 상태인지 확인하세요.\");const r=e=>String(e).padStart(2,\"0\"),i=e=>e.getFullYear()+r(e.getMonth()+1)+r(e.getDate())+r(e.getHours())+r(e.getMinutes()),a=async(e,t)=>{const o=new URLSearchParams({type:\"alertlist_load\",pagetype:\"setup\",idx:n,pageid:\"alertlist\",preset:\"0\",\"ver[server]\":\"20240607\",\"ver[socket]\":\"20240607\",lastdate:\"\",\"filter[start]\":e,\"filter[end]\":t,\"filter[min]\":\"0\",\"filter[type]\":\"all\",\"filter[search]\":\"\"});return(await fetch(\"https://weflab.com/api/\",{method:\"POST\",credentials:\"include\",headers:{\"Content-Type\":\"application/x-www-form-urlencoded; charset=UTF-8\",\"X-Requested-With\":\"XMLHttpRequest\"},body:o.toString()})).json()};try{const t=await fetch(e+\"/cursor\",{method:\"POST\",headers:o}).then(e=>e.json());if(!t.ok)return void alert(\"커서 조회 실패: \"+(t.error||\"\"));const n=t.last_idx||0;console.log(\"[WefImport] 커서 last_idx =\",n);const r=i(new Date(Date.now()-31536e7));let s=i(new Date(Date.now()+1728e5));const l=new Map;let c=0,d=!1;for(;c<60;){c++;const e=await a(r,s);if(\"success\"!==e.result)return alert(\"⚠️ 크롤 실패 (세션 만료?). weflab에 다시 로그인 후 시도하세요.\"),void console.log(\"[WefImport] 응답:\",e);const t=e.data||[];if(!t.length)break;for(const e of t)l.set(Number(e.idx),e);const o=t[t.length-1];if(Number(o.idx)<=n){d=!0;break}if(t.length<100)break;s=o.create_time.replace(/[-: ]/g,\"\").slice(0,12)}const p=[...l.values()].filter(e=>Number(e.idx)>n);if(console.log(`[WefImport] 크롤 ${l.size}건 (${c}페이지), 신규 ${p.length}건${d?\"\":\" ⚠️커서 미도달(더 있을 수 있음)\"}`),!p.length)return void alert(\"신규 룰렛 결과가 없습니다. (last_idx=\"+n+\")\");const m=await fetch(e+\"/stage\",{method:\"POST\",headers:o,body:JSON.stringify({records:p})}).then(e=>e.json());if(!m.ok)return void alert(\"대기열 적재 실패: \"+JSON.stringify(m));const f=m.rpc||{};console.log(\"=== [WefImport] 대기열 적재 결과 ===\",f),(f.unmapped_items||[]).length&&console.log(\"⚠️ 미매핑(무시됨):\",f.unmapped_items.join(\", \"),\"→ 기록하려면 admin 룰렛연동에서 매핑\"),alert(`✅ 입력 대기열에 추가됨\n\n크롤 ${m.raw_records}건 → 스핀 ${m.expanded_spins}개\n대기 항목: 처리 ${f.staged||0}개\n2배 적용: ${f.doubled||0} / 만료: ${f.doubled_expired||0}\n필러 제외: ${f.skipped_filler||0}\n미매핑(무시): ${(f.unmapped_items||[]).length}\n커서: ${f.prev_last_idx} → ${f.new_last_idx}\n\nadmin \"입력 확인\" 탭에서 검토 후 승인하세요.`)}catch(e){alert(\"오류: \"+e.message),console.error(\"[WefImport]\",e)}})();"; // 빌드 시 주입
