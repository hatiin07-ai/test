...
const WG_BOOKMARKLET = "javascript:(async()=>{const e=\"https://uootyovhokziobarpeed.supabase.co/functions/v1/wef-import\",t=\"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVvb3R5b3Zob2t6aW9iYXJwZWVkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI4NTU2MDIsImV4cCI6MjA5ODQzMTYwMn0.lQm3PLyoj5ELktvcuUuVbidh4xmv8W2Oo4uUddQrJk4\",r={\"Content-Type\":\"application/json\",\"X-Import-Token\":\"j7W7edUfwOCoKa08DyyiXjtNVLtUG-HpdTTlDzRmzQg\",apikey:t,Authorization:\"Bearer \"+t};if(\"weflab.com\"!==location.hostname)return void alert(\"⚠️ weflab.com 후원관리 페이지에서 실행하세요.\");const o=(document.cookie.match(/login_idx=([^;]+)/)||[])[1];if(!o)return void alert(\"⚠️ login_idx 없음. weflab 로그인 상태인지 확인하세요.\");const n=e=>String(e).padStart(2,\"0\"),a=e=>e.getFullYear()+n(e.getMonth()+1)+n(e.getDate())+n(e.getHours())+n(e.getMinutes()),s=async(e,t)=>{const r=new URLSearchParams({type:\"alertlist_load\",pagetype:\"setup\",idx:o,pageid:\"alertlist\",preset:\"0\",\"ver[server]\":\"20240607\",\"ver[socket]\":\"20240607\",lastdate:\"\",\"filter[start]\":e,\"filter[end]\":t,\"filter[min]\":\"0\",\"filter[type]\":\"all\",\"filter[search]\":\"\"});return(await fetch(\"https://weflab.com/api/\",{method:\"POST\",credentials:\"include\",headers:{\"Content-Type\":\"application/x-www-form-urlencoded; charset=UTF-8\",\"X-Requested-With\":\"XMLHttpRequest\"},body:r.toString()})).json()};try{const t=await fetch(e+\"/cursor\",{method:\"POST\",headers:r}).then(e=>e.json());if(!t.ok)return void alert(\"커서 조회 실패: \"+(t.error||\"\"));const o=t.last_idx||0;console.log(\"[WefImport] 커서 last_idx =\",o);const n=a(new Date(Date.now()-31536e7));let i=a(new Date(Date.now()+1728e5));const l=new Map;let c=0,d=!1;for(;c<60;){c++;const e=await s(n,i);if(\"success\"!==e.result)return alert(\"⚠️ 크롤 실패 (세션 만료?). weflab에 다시 로그인 후 시도하세요.\"),void console.log(\"[WefImport] 응답:\",e);const t=e.data||[];if(!t.length)break;for(const e of t)l.set(Number(e.idx),e);const r=t[t.length-1];if(Number(r.idx)<=o){d=!0;break}if(t.length<100)break;i=r.create_time.replace(/[-: ]/g,\"\").slice(0,12)}const p=[...l.values()].filter(e=>Number(e.idx)>o);if(console.log(`[WefImport] 크롤 ${l.size}건 (${c}페이지), 신규 ${p.length}건${d?\"\":\" ⚠️커서 미도달(더 있을 수 있음)\"}`),!p.length)return void alert(\"신규 룰렛 결과가 없습니다. (last_idx=\"+o+\")\");const m=await fetch(e+\"/import\",{method:\"POST\",headers:r,body:JSON.stringify({dry_run:!0,records:p})}).then(e=>e.json());if(!m.ok)return void alert(\"미리보기 실패: \"+JSON.stringify(m));const f=m.rpc;console.log(\"=== [WefImport] 미리보기 (기입 예정) ===\"),console.table((f.rewards||[]).map(e=>({\"닉네임\":e.nickname,\"아이디\":e.user_id,\"항목\":e.item,\"개수\":e.qty,task_type:e.task_type_id}))),(f.created_task_types||[]).length&&console.log(\"신규 항목 생성 예정(미분류):\",f.created_task_types.map(e=>e.name)),(f.created_members||[]).length&&console.log(\"신규 멤버 생성 예정:\",f.created_members.map(e=>e.user_id));const u=`📋 미리보기 (상세는 콘솔 확인)\\n\\n크롤: ${m.raw_records}건 → 스핀 ${m.expanded_spins}개\\n보상 기입 예정: ${(f.rewards||[]).length}종\\n2배 적용: ${f.doubled} / 만료: ${f.doubled_expired}\\n필러 제외: ${f.skipped_filler} / 2배트리거: ${f.skipped_double}\\n신규항목 생성: ${f.unmapped_autocreated} / 신규멤버: ${(f.created_members||[]).length}\\n\\n실제로 기입할까요?`;if(!confirm(u))return void console.log(\"[WefImport] 취소됨 (미기입)\");const h=await fetch(e+\"/import\",{method:\"POST\",headers:r,body:JSON.stringify({dry_run:!1,records:p})}).then(e=>e.json());if(!h.ok)return void alert(\"기입 실패: \"+JSON.stringify(h));console.log(\"=== [WefImport] 기입 완료 ===\",h.rpc),alert(`✅ 기입 완료\\n\\n보상 ${(h.rpc.rewards||[]).length}종\\n커서: ${h.rpc.prev_last_idx} → ${h.rpc.new_last_idx}`)}catch(e){alert(\"오류: \"+e.message),console.error(\"[WefImport]\",e)}})();"; // 빌드 시 주입
...
async function wgInit() {
  ...
  if (curEl) curEl.textContent = curRow ? curRow.value : "0";
  if (wgConfig && document.getElementById("wgStartIdx")) {
    document.getElementById("wgStartIdx").value = wgConfig.start_idx ?? 0;
  }
  // 북마클릿
